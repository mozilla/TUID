...
class RolloverIndex(object):
        ...
        if queue == None:
            ...
            if not best or rounded_timestamp > best.date:
                if rounded_timestamp < wrap(candidates[-1]).date:
                    es = self.cluster.get_or_create_index(
                        read_only=False,
                        alias=best.alias,
                        index=best.index,
                    ...
                    try:
                        es = self.cluster.create_index(
                            create_timestamp=rounded_timestamp, kwargs=self.settings
                        )
            ...
            threaded_queue = es.threaded_queue(
        ...
        for c in candidates:
            ...
            if timestamp + self.rollover_interval < Date.today() - self.rollover_max:
                ...
                try:
                    ...
                    Log.warning(
                        "could not delete index {{index}}", index=c.index, cause=e
                    )
    ...
    def copy(
        self, keys, source, sample_only_filter=None, sample_size=None, done_copy=None
    ):
        """
            ...
            try:
                with timer:
                    for rownum, line in enumerate(
                        ...
                        row, please_stop = fix(
                            rownum, line, source, sample_only_filter, sample_size
                        )
                        if row == None:
                            continue
...
def fix(rownum, line, source, sample_only_filter, sample_size):
    ...
    if rownum == 0:
        ...
        if (
            sample_only_filter
            and Random.int(int(1.0 / coalesce(sample_size, 0.01))) != 0
            and jx.filter([value], sample_only_filter)
        ):
    ...
    if shorter_length > MAX_RECORD_LENGTH:
        ...
        if source.name == "active-data-test-result":
            if result_size > MAX_RECORD_LENGTH:
                Log.warning(
                    "Epic test failure in {{name}} results in big record for {{id}} of length {{length}}",
                    id=value._id,
                    name=source.name,
            ...
            Log.warning(
                "Monstrous {{name}} record {{id}} of length {{length}}",
                id=value._id,
                name=source.name,
                length=shorter_length,
            )
