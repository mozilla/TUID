...
def format_cube(decoders, aggs, start, query, select):
    ...
    cube = Cube(
        query.select,
        sort_using_key(
            new_edges, key=lambda e: e.dim
        ),  # ENSURE EDGES ARE IN SAME ORDER AS QUERY
...
def format_table(decoders, aggs, start, query, select):
        ...
        if query.sort and not query.groupby:
            ...
            for row, coord, agg in aggs_iterator(aggs, decoders):
                ...
                while coord != missing_coord:
                    record = [
                        d.get_value(missing_coord[i]) for i, d in enumerate(decoders)
                    ]
                    for s in select:
    ...
    return Data(meta={"format": "table"}, header=header, data=list(data()))
...
set_default(
    format_dispatch,
    {
        None: (
            format_cube,
            format_table_from_groupby,
            format_cube_from_aggop,
            "application/json",
        ),
        ...
        "table": (
            format_table,
            format_table_from_groupby,
            format_table_from_aggop,
            "application/json",
        ),
        "list": (
            format_list,
            format_list_from_groupby,
            format_list_from_aggop,
            "application/json",
        ),
