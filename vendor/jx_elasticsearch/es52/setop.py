        ...
        if isinstance(select.value, LeavesOp) and isinstance(
            ...
            for c in leaves:
                full_name = concat_field(
                    select.name, relative_field(untype_path(c.names["."]), term.var)
                )
                if c.jx_type == NESTED:
            ...
            if leaves:
                if s_column == ".":
                    ...
                    for c in leaves:
                        if (
                            len(c.nested_path) == 1
                        ):  # NESTED PROPERTIES ARE IGNORED, CAPTURED BY THESE FIRT LEVEL PROPERTIES
                        ...
                        if len(c.nested_path) == 1:
                            ...
                            if not nested_filter:
                                ...
                                for k in filters[0].keys():
                                    filters[0][k] = None
                                set_default(
                                    filters[0], es_and([where, es_or(nested_filter)])
                                )
                            ...
                            if nested_path not in nested_selects:
                                ...
                                pull = accumulate_nested_doc(
                                    ...
                                    Variable(
                                        relative_field(
                                            s_column, unnest_path(nested_path)
                                        )
                                    ),
                                )
                                new_select.append(
                                ...
                                nested_selects[
                                    nested_path
                                ].nested.inner_hits.stored_fields += [c.es_column]
            else:
                new_select.append(
                    {
                        "name": select.name,
...
def format_table(T, select, query=None):
    ...
    if isinstance(query.select, Mapping) and not isinstance(
