...
def override(func):
    ...
    def raise_error(e, packed):
        ...
        if err.startswith(func_name) and (
            "takes at least" in err or "required positional argument" in err
        ):
    ...
    if "kwargs" not in params:
        ...
        def w_constructor(*args, **kwargs):
            if "kwargs" in kwargs:
                ...
                packed = params_pack(
                    params, kwargs, dict_zip(params[1:], args[1:]), defaults
                )
            try:
        ...
        def w_kwargs(*args, **kwargs):
            if len(args) == 1 and len(kwargs) == 0 and isinstance(args[0], Mapping):
                ...
                # PUT args INTO kwargs
                packed = params_pack(
                    params, kwargs, dict_zip(params, args), kwargs["kwargs"], defaults
                )
            else:
