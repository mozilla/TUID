    ...
    with RegisterThread():
        try:
            ...
            for a in ands:
                ...
                annotated_files = coalesce(annotated_files, a.eq.annotated_files)
                commit = coalesce(commit, a.eq.commit)
            ...
            if len(paths) == 0:
                ...
                with Timer("tuid internal response time for {{num}} files", {"num": len(paths)}):
                    response, completed = service._get_tuids(
                    revision=rev, files=paths, annotated_files=annotated_files, commit=commit, repo=branch_name
                )
...
if __name__ in ("__main__",):
    ...
    try:
        ...
        Log.note("Started TUID generation Service")
    ...
    if config.flask2:
        if config.flask2.port and config.args.process_num:
            config.flask2.port += config.args.process_num
        Log.note("Running Flask...")
        flask_app.run(**config.flask2)
