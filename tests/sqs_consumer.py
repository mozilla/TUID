...
def one_request(request, please_stop):
    ...
    with Timer(
        "Make TUID request from {{timestamp|datetime}}", {"timestamp": request.meta.request_time}
    ):
        try:
            result = http.post_json("http://localhost:5000/tuid", json=request, timeout=30)
...
def queue_consumer(pull_queue, please_stop=None):
    ...
    while not please_stop:
        ...
        if next_request > now:
            Log.note(
                "Next request in {{wait_time}}", wait_time=Duration(seconds=next_request - now)
            )
            Till(till=next_request).wait()

        Thread.run("request " + text(request_count), one_request, request)
        request_count += 1
        queue.commit()
